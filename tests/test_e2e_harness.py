...
def test_claude_needs_no_headless_run_args() -> None:
    # claude -p runs headless from any directory with no extra flags.
    assert harness.agent_run_args("claude") == ()


def test_codex_declares_headless_run_args() -> None:
    # codex exec needs its own CLI flags to run non-interactively from an arbitrary
    # (non-git) working directory: skip the git-repo gate and pin a non-interactive
    # sandbox. These pass through as node `args` (#11 acceptance 3), so the node SHAPE
    # stays symmetric across agents — only the agent-supplied flags differ.
    run_args = harness.agent_run_args("codex")
    assert "--skip-git-repo-check" in run_args
    assert "--sandbox" in run_args
